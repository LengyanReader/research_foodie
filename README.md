...
- **Track C · evidence-grounded QA** (`qa` scenario family in bench_eval, Sessions 17–19): corpus-anchored + **Qasper external-author gold QAs** + **SciQ provided-context MCQs** (`ctx` routing, no downloads). Learnable-loop: survey graph answers *about* a paper → `seed_id`/`ctx` scenarios route to a grounded **extractive-answer node** (`tools/pipeline/answer.py`; ~15–20 s/answer). **QA panel n=15: correctness 4.73 · groundedness 4.40 · 14/15 correct** — extract/context paths **10/10 perfect**; only synthesis/taxonomy questions (QA-3) stay below 5 (they need multi-paper S_write — documented boundary); 5 new corpus papers added (SBERT, UTCNN, NLP4IF, QG, MPAD).
