...
- **L-1. S_write relevance re-rank (mirror PaperQA2 RCS)** — before per-paper claim extraction, score sentence/paragraph windows against the question(s) with a cheap lexical (BM25-style / overlap + section-prior) ranker; inject only top-k windows into the extraction LLM call. *Rationale:* our worst retrieval gap — we feed the raw first ≤20 000 chars of each paper instead of the *relevant* passages. *Accept:* on P-A/P-B/P-C + QA-1..5, extract/context QA keeps 12/13; groundedness 4.45 not regressed; claims-wall-sheet verbatim 5-gram gate 34/34; and on a topic where the answer is mid-paper (e.g. QA-3 counterpart), measured pass shifts from cold-miss to hit without taxonomy surgery. *Fallback:* keep raw window + section log, so worst case = no harm. — 2026-09-20 **implemented** (`tools/pipeline/rerank.py` Okapi-BM25; wired into `graph._write` per-paper with `source_windows` provenance; grounding still verified against FULL md so the gate is never weakened). Unit-verified (`test_evolution` L-1 fallback modes: full/bm25-doc-order/raw + budget) and mock integration 34/34 with `source_windows` logged. *Live P-A..C/QA re-measure pending a real cadence.*
- **L-2. S_org perspective decomposition (STORM-inspired, prompt-only)** — `_org` first asks for 3–5 reader perspectives (with the user-provided question as anchor), then answers each per-paper-window, then produces the outline (STORM's max_turn×(max_perspective+1) QA budget, re-implemented at low scale). *Accept:* outline `key_points` coverage judged ≥2 on 2 previously-thin topics, family Total not lowered; no new determinism/lint regressions. — 2026-09-20 **implemented** (`graph._org` perspective rail before the outline call; ≤5 perspectives parsed, injected as "Perspectives to cover", parse-fail → `[]` no-harm; `state.perspectives`). Mock asserts perspectives=3; deterministic. *Two-topic coverage judgment pending a real cadence.*
- **L-3. Judge median-of-3 aggregation** — run each bench judge call 3× (3 flat retries exist in LLMClient) and take the **median** of the 4-axis verdict; keep single-call in pipeline runtime. *Accept:* P-A judge sd < 0.40 across 3 rounds (from 0.53), P-B stays 0.00, reported family means restated with medians. — 2026-09-20 **implemented** (`bench_eval.median_bench` per-criterion median over `--median-rounds`, `score_survey` temperature param, `variance_run` median-round wiring + single-call baseline kept separate so health σ stays valid). Unit-verified (median math, errored-round skip, empty-input). *Real sd<0.40 re-measure pending live rounds.*
- **L-4. (Easy) wire `citation-verification` locally installed skill** into the assembly guidance (agent-side checklist), no code change; and expose the 220-survey rubric anchors as judge few-shot calibrators (skip — needs PDF parse, park in Phase R). — 2026-09-20 **implemented** (agent-side checklist added to `docs/setup-runbook.md` §3, pointing at the `citation-verification` skill; `tools/citation-verify/` scripts stay reference-only — no code change, as scoped).
...
- **L-6. (optional) Evaluation-one-command** — bundle `bench_eval --scenarios` + `variance_run` + `health_check` (below) behind a single `tools/eval/self_check.ps1` runner so the cadence is one command. *Accept:* one command reproduces the health report; exit code reflects GREEN/WARN/FAIL. — 2026-09-20 **implemented** (`tools/eval/self_check.py` + repo-root `self_check.ps1`: unit→integration→cadence, exit = max level). Verified offline GREEN in ~13 s.
