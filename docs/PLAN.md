...
- `Updated`: 2026-09-16
- `Status`: **Tasks A–D complete. P1 tooling complete; P2 minimal vertical GREEN + framework integration + generalization; benchmark-style evaluation pilot done (Session 11); multi-paper evidence synthesis P0 GREEN (Session 12)** — MinerU ✓, PaddleOCR ✓ (K7 closed), LLM client ✓ (default `opencode` backend, `opencode/big-pickle`, `--auto` for headless robustness; OpenAI-compat; **Ollama backend removed 2026-09-16**); `langgraph` 1.1.10 installed; `tools/pipeline/` P2 live: **S_lit pluggable discovery rails (seed | arXiv API | orx CLI; arXiv verified reachable 2026-09-16)** + **multi-paper evidence pool `resolved_evidence()` (3 local papers: Liang `2304.02819` · Weber-Wulff `2306.15666` · GLTR `1906.04043`) + per-paper grounded claims (`paper_id` attribution)** + full graph + L6 deterministic gate (+ informational `multi_paper` check) + code-fence-tolerant JSON parsing; **STORM-style outline + DAS-Bench-style AI judge gate integrated (Session 9)**; **mock 23/23 + real `opencode/big-pickle` 23/23 (110.8 s, score 1.0, 4 grounded claims); DAS-16 pilot: P-A Total 3.06 with papers=3 / cited=3 (was 2.62 single-paper); 001 precision-fixed to no-evidence**. Next: raise draft length/section-count to survey-grade TSQ/MAR + P3 judge calibration against DAS-16 + DAS-2M/≥300B judge for a compliant submission.
...
- ~~**P0. Multi-paper evidence in S_lit** — extend `S_lit` to fetch + parse top-k full texts (arXiv live rail + MinerU `-m txt` windows, reusing the K12 workaround) so the graph can synthesize *across* papers.~~ **DONE Session 12**: `resolved_evidence()` evidence pool (3 local parses: Liang `2304.02819` · Weber-Wulff `2306.15666` · GLTR `1906.04043`); per-paper grounded claim extraction with `paper_id` attribution; pilot P-A now papers=3/cited=3, Total 3.06 (was 2.62), Multi-Reference + Citation-Distribution ≥3 ✓; TSQ/MAR still below survey floor → the *draft-depth* lever (longer, section-scoped synthesis) is the next step. New row 15 (Session 12) below.
