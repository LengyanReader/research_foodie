...
## 2026-09-23 — Session 35: 知识库 + 阅读器（数据/知识管理落地 · 工作台产品化 Phase H）(knowledge library + reader)

**Why:** 用户三项指令——(1) **数据/知识管理**（底层 + 界面都要）把 survey/bench 手稿登记为知识条目；(2) markdown/PDF **页内阅读**；(3) 页面体现**明确产品形态、可用、好用**，作为后续迭代依据。

1. **底层（`tools/web/knowledge.py`，H-1）** — SQLite（stdlib `sqlite3`，无新依赖）索引 + 用户态：`items(key,kind,title,question,path_md,path_pdf,path_preprint,status,mode,verdict,judge,claims,papers,elapsed_s,tags,favorite,created_at,updated_at)` 于 `_eval_out/knowledge.db`。**文件是事实源头**：`sync()` 枚举 `_eval_out/manuscripts/`+`mock_manuscripts/`，用 run 日志 `[survey-result]`（basename 匹配）富化 kind/mode/verdict/claims/papers/elapsed 并 upsert；磁盘已删条目自动清除。CRUD：`set_tags`/`toggle_favorite`/`delete_item`——删除**只作用于该条目声明的文件路径**（`files_of()` 解析 resolve 后逐个 unlink），run 日志永不触碰。
2. **知识库页（`/library`，H-2）** — 卡片网格（`auto-fill`），每卡 = kind 徽章 + 状态 chip + 收藏星 + 问题/标题 + 元数据行（mode/verdict/claims/papers/elapsed）+ 可编辑标签 chips（Enter 增 / × 删）+ 动作（阅读 / MD / PDF / 删除·confirm）。顶部：搜索框（实时）+ 筛选 pill（全部/收藏/已通过/Mock/Real，`data-*` 客户端过滤）+ 重新扫描 + 计数。收藏/标签/删除走 JSON API；标签变更整页刷新（稳妥优先，后续可升级为原位渲染）。
3. **阅读器（`/read/{key}`，H-3）** — chrome：返回键 + 标题 + tab（正文 / PDF）+ 新窗口/下载。**正文**用 `markdown_it`（已装，offline）渲染 `.kprose` 排版（贴合 warm-calm token），`html=False` 禁原生 HTML 直出（防注入）；**PDF** 用浏览器原生 viewer `<iframe>`（零外部网络）+ `download`。survey 条目顶部横幅：问题 + verdict chip + mode/claims/papers/elapsed。三处 fix：`MarkdownIt("js-default", {...})` 需显式 preset（否则 `maxNesting` KeyError）；`linkify` 需 linkify-it-py（未装 → 关闭）；`c.fetchall()` 应为 cursor（`cur.fetchall()`）。
4. **接线** — nav 新增"知识库"pill（`nav_lib`，en/zh 全部进 `UI` 并走 i18n）；`.lib-grid/.kcard/.kfav/.ktag/.fpill/.kprose/.reader-head/.rtabs/.kviewer` 等 CSS 沿用 warm-calm token（暖纸白卡 + 蓝绿 accent + 人文无衬线 + mono 只在 code/pre/#tail）；`/manuscripts` 保持向后兼容（阅读器 iframe 与旧链接复用同一 FileResponse）。

**Verification:** `ast.parse` OK ✓；原有 i18n 断言脚本全绿 ✓；新增 library smoke（TestClient）通过：sync **45 条目**、fav toggle 两次、tags 添加"方法论"再移除、en+zh `/library`、`/read/{key}`（正文 + PDF pane 齐全）、PDF GET `application/pdf` 200 ✓；测试残留 favorited/tags 已清零；8787 重启后 live：`/library` 200 · 62KB，`/read/gpt-detectors-…` 200（kprose + pane-pdf + iframe 命中）✓。
