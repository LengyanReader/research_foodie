...
- **Track C · 证据接地 QA**（`qa` 场景族，Session 17-18）：语料锚定题（GLTR/Liang/Weber-Wulff）+ **Qasper 外部作者 gold 题**。可学习闭环修复：综述图答"论文"而非"问题"，故 `seed_id` 场景改走新的**接地抽取式回答节点**（`tools/pipeline/answer.py`）——QA-6/QA-7 从 c 1-2 → **5/5、gold 2/2**（约 16 s/问）；QA-3 分类学题 c 1→3（综合型仍需多稿合成，已记录边界）。**QA 试点 n=7：correctness 4.43 · groundedness 4.57 · 抽取路径 3/3 gold 完美**；新增 2 篇语料论文（SBERT 1908.10084、UTCNN 1611.03599）。
